...
class PromptOllama(FlowFileTransform):
    class Java:
        implements = ["org.apache.nifi.python.processor.FlowFileTransform"]

    class ProcessorDetails:
        version = "2.0.0.dev0"
        description = "Submits a prompt to Ollama, writing the results either to a FlowFile attribute or to the contents of the FlowFile"
        tags = [
            "text",
            "ollama",
            "machine learning",
        ...
        dependencies = ["langchain==0.1.2", "jsonpath-ng"]

    HOST = PropertyDescriptor(
        name="Ollama Host",
    ...
    PROMPT = PropertyDescriptor(
        name="Prompt",
        description="""The prompt to issue to Ollama. This may use FlowFile attributes via Expression Language and may also reference the FlowFile content by using the literal
    ...
    TEMPERATURE = PropertyDescriptor(
        name="Temperature",
        description="""The Temperature parameter to submit to Ollama. A lower value will result in more consistent answers while a higher value will result in a more creative answer.
    ...
    def transform(self, context, flowFile):
        ...
        host = context.getProperty(self.HOST).evaluateAttributeExpressions(flowFile).getValue()
